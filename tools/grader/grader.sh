...
# Ingest JSON
resp=$(curl -s -X POST "$BASE_URL/order/tracking" -H "Content-Type: application/json"   -d '{"event":[{"orderId":"A1","status":"RECOGIDO_EN_ALMACEN","eventTs":"2025-01-01T10:00:00Z"}]}' )
echo "$resp" | grep -q '"accepted":true' && pass "JSON ingestion accepted" || fail "JSON ingestion failed"

# Ingest XML
resp=$(curl -s -X POST "$BASE_URL/order/tracking" -H "Content-Type: application/xml"   -d '<events><event><orderId>A2</orderId><status>RECOGIDO_EN_ALMACEN</status><eventTs>2025-01-01T10:00:00Z</eventTs></event></events>')
