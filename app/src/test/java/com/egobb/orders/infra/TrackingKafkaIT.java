...
class TrackingKafkaIT {
  ...
  @Autowired ProcessTrackingUseCase useCase;
  ...
  void fanout_to_kafka_and_autoconsume_in_order_per_orderId() {
    ...
    Mockito.verify(this.trackingProcessor, times(batch.size()))
        .processOne(this.eventCaptor.capture());
  ...
  private static void assertOrderedByTimestamp(List<TrackingEvent> events) {
    ...
    assertThat(events)
        .as("Events for a given orderId must be processed in temporal order (FIFO per partition)")
