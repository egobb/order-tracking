...
public class IngestTrackingEventsUseCase {
	...
	public IngestTrackingEventsUseCase(OrderTimelineRepository repository, EventAppender appender, StateMachine sm) {
	...
	public List<Result> ingest(List<TrackingEvent> events) {
		...
		for (final TrackingEvent e : events)
			out.add(this.processOne(e));
		return out;
	}
	...
	private Result processOne(TrackingEvent e) {
		...
		if (!this.sm.canTransition(tl.lastStatus(), e.status())) {
			return Result.rejected(e.orderId(), "Transition not allowed from " + tl.lastStatus() + " to " + e.status());
		}
